...
public class AliRtcChatActivity extends AppCompatActivity {
    ...
    private void joinChannel() {
        ...
        userInfo.setAppId(mData.getAppID()); //应用AppID
        userInfo.setNonce(mData.getNonce()); //随机码。需要加上前缀AK-
        userInfo.setGslb(new String[]{"https://rgslb.rtc.aliyuncs.com"});  //服务地址，当前请使用["https://rgslb.rtc.aliyuncs.com"]
        userInfo.setTimestamp(mData.getTimeStamp()); //令牌过期时间戳
        userInfo.setToken(mData.getToken());     //加入频道Token
        userInfo.setChannelId(mData.getChannelId()); //频道ID
        userInfo.setUserId(mData.getUserId());  //用户ID
