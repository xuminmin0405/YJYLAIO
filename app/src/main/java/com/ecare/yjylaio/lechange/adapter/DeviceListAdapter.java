...
import com.ecare.yjylaio.lechange.view.SpacesItemDecoration;
...
public class DeviceListAdapter extends RecyclerView.Adapter<DeviceListAdapter.DeviceListHolder> {
    ...
    public void onBindViewHolder(@NonNull final DeviceListHolder holder, final int position) {
        ...
        if (datas.get(position).channels!=null&&datas.get(position).channels.size() > 1) {
            //多通道
        ...
        } else if (datas.get(position).channels!=null&&datas.get(position).channels.size() == 0){
            //多通道NVR,但是没有通道数
            holder.ivDetail.setVisibility(View.VISIBLE);
            ...
            holder.rlDetail.setVisibility(View.VISIBLE);
            holder.rcvChannel.setVisibility(View.GONE);
            if ("online".equals(datas.get(position).status)&&datas.get(position).channels!=null&&datas.get(position).channels.size()>0) {
